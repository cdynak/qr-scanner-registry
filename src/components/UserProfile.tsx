...
export function UserProfile({ user, className, showEmail = true, size = "md" }: UserProfileProps) {
  const sizeClasses = {
    sm: {
      avatar: "w-8 h-8",
      text: "text-sm",
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      ...
      <div className="flex-1 min-w-0">
        <div className={cn("truncate", classes.name)}>{user.name}</div>
        {showEmail && <div className={cn("truncate", classes.email)}>{user.email}</div>}
      </div>
    </div>
  );
}
